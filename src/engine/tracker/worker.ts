...
export interface InitMessage {
  ...
  procWidth: number;
  procHeight: number;
  /** URL of the WASM kernels; defaults to '/tracker.wasm'. */
  wasmSrc?: string;
}
...
let kernelsReady: Promise<void> | null = null;

function loadKernels(wasmSrc: string): Promise<void> {
  if (!kernelsReady) {
    kernelsReady = (async () => {
      try {
...
self.onmessage = async (event: MessageEvent<InitMessage | FrameMessage>) => {
  ...
  if (msg.type === 'init') {
    await loadKernels(msg.wasmSrc ?? '/tracker.wasm');
