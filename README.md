...
表示コンテンツは HUD の「コンテンツ」セレクタで **キューブ / 画像 / 動画** を切り替えられます（画像・動画は端末のファイルを選択。ターゲット矩形に contain フィットで配置）。緑の枠・半透明プレーン・3 軸・黄色い特徴点は**トラッキング判定を可視化するデバッグ表示**で、「判定を表示」チェックでまとめて ON/OFF できます。
